...
get_block_height() {
    globalHeight=$(curl -s -X POST 'https://ethereum.blockpi.network/v1/rpc/0cb922caf1e980040cbecca069e07c542d8fa373' -H 'Content-Type: application/json' -d '{"jsonrpc": "2.0","method": "eth_blockNumber","params": [],"id": 1}' | jq -r ".result" | tr 'a-f' 'A-F')
    LocalHeight=$(curl -Ls -X POST 'http://127.0.0.1:31301' -H 'Content-Type: application/json' -d '{"jsonrpc": "2.0","method": "eth_blockNumber","params": [],"id": 1}' | jq -r ".result" | tr 'a-f' 'A-F')
}
